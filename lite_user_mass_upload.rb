  ...
  def self.process(file_name)
    ...
    File.open("lite_users_mass_upload.csv", "w") do |file|
      ...
      billing_user_hash.each do |key, lite_user_hash|
        ...
        file.puts "Prev,,,New Product,Lite Package Price Change,Lite Package includes 10000 email credits per month. $0.0001 per email thereafter.,,,,,7/01/2013,,7/01/2013,7/01/2013,7/01/2013,,#{lite_user_hash['Product.SKU']},Self-serve Email Plans,#{lite_user_hash['ProductRatePlan.Id']},Lite Package,,Lite Package,Lite Package includes 10000 email credits per month. $0.0001 per email thereafter.,1,1,,,,,,,,#{lite_user_hash['Subscription.TermType']},,,,,,,,,"
